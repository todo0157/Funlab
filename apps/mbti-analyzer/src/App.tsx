...
import { analyzeMBTI } from './services/analysisService';
import type { MBTIAnalysisResult, AnalysisState, ParsedChat, AnalysisTier } from './types/mbti';
...
function App() {
  ...
  const handleFileSelect = useCallback(async (content: string) => {
    ...
    try {
      ...
      if (parsed.participants.length < 1) {
        throw new Error('대화 참여자가 없어요. 올바른 카카오톡 대화 파일을 업로드해주세요.');
      }
  ...
  const handleTierSelect = useCallback(async (tier: AnalysisTier) => {
    ...
    try {
      const analysisResult = await analyzeMBTI(parsedChat, tier);
        ...
        {analysisState !== 'complete' && analysisState !== 'tierSelection' && (
          <div className="text-center mb-8">
            <h1 className="text-3xl sm:text-4xl font-bold text-gray-900 dark:text-white mb-4">
              <span className="gradient-text">MBTI 대화 스타일</span> 분석기
            </h1>
            <p className="text-gray-600 dark:text-gray-400 max-w-lg mx-auto">
              카카오톡 대화를 AI가 분석해서 대화 스타일 기반 MBTI를 예측해드려요
            </p>
          </div>
        )}
          ...
          <div className="flex flex-col items-center justify-center py-16 space-y-6">
            ...
            <div className="text-center">
              <p className="text-lg font-semibold text-gray-900 dark:text-white mb-1">
                {analysisState === 'parsing' ? '대화를 읽고 있어요...' : 'AI가 MBTI를 분석 중이에요...'}
              </p>
