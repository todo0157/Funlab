...
import { QuizData, QuizQuestion } from '../../types/bestfriend';
    ...
    >
      <div className="text-center mb-6">
        <h2 className="text-xl font-bold text-gray-900 dark:text-white">
          <span className="text-violet-500">{quizData.targetName}</span> 찐친 테스트 편집
        </h2>
        ...
        {questions.map((question, qIndex) => (
          ...
          >
            <div className="flex items-start justify-between mb-4">
              <span className="flex items-center justify-center w-8 h-8 bg-violet-100 dark:bg-violet-900/50 text-violet-600 dark:text-violet-400 rounded-full font-bold text-sm">
                {qIndex + 1}
              </span>
              <button
            ...
            {editingIndex === qIndex ? (
              <textarea
                ...
                className="w-full p-3 bg-gray-50 dark:bg-gray-700 rounded-xl border-2 border-gray-200 dark:border-gray-600 focus:border-violet-500 dark:focus:border-violet-500 outline-none text-gray-900 dark:text-white resize-none"
                rows={2}
              />
            ) : (
      ...
      <div className="flex flex-col sm:flex-row gap-3 mt-6">
        <motion.button
          onClick={handleSave}
          className="flex-1 py-4 bg-gradient-to-r from-violet-500 to-indigo-500 text-white font-bold rounded-xl hover:from-violet-600 hover:to-indigo-600 transition-all shadow-lg"
