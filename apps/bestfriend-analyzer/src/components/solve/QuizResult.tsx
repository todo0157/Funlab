...
import { QuizData, QuizResult as QuizResultType } from '../../types/bestfriend';
...
const GRADE_COLORS = {
  ...
  A: 'from-violet-400 to-indigo-500',
...
export function QuizResultComponent({ quizData, result, onRetry }: QuizResultProps) {
  ...
  const handleShare = async () => {
    const shareData = {
      title: `${quizData.targetName} 찐친 테스트 결과`,
      text: `${quizData.targetName} 찐친 테스트에서 ${result.totalQuestions}문제 중 ${result.score}문제 맞춤! (${percentage}%) - 나도 도전해보기!`,
    ...
    >
      <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-lg text-center">
        ...
        >
          ...
          <p className="text-lg text-violet-500 font-medium mb-1">{percentage}%</p>
        ...
        >
          <p className="text-sm text-gray-500 dark:text-gray-400">
            <span className="font-semibold text-violet-500">{quizData.targetName}</span>
            {' '}찐친 테스트
          </p>
        ...
        >
          <button
            onClick={handleShare}
            className="w-full py-4 bg-gradient-to-r from-violet-500 to-indigo-500 text-white font-bold rounded-xl hover:from-violet-600 hover:to-indigo-600 transition-all shadow-lg"
          >
            결과 공유하기
          </button>
          <button
