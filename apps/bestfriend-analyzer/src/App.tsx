...
import type { AppState, ParsedChat, QuizData, QuizResult, AnalysisTier } from './types/bestfriend';
  ...
  const handleFileSelect = useCallback(async (content: string) => {
    ...
    try {
      ...
      if (parsed.participants.length < 2) {
        throw new Error('대화 참여자가 2명 이상이어야 해요.');
      }
        ...
        {(appState === 'idle' || appState === 'error') && (
          <div className="text-center mb-8">
            <h1 className="text-3xl sm:text-4xl font-bold text-gray-900 dark:text-white mb-4">
              <span className="gradient-text">찐친</span> 테스트
            </h1>
            <p className="text-gray-600 dark:text-gray-400 max-w-lg mx-auto">
              친구가 나를 얼마나 아는지 테스트해보세요!
            </p>
          </div>
        )}
        ...
        {isLoading && (
          <div className="flex flex-col items-center justify-center py-16 space-y-6">
            <div className="relative">
              <div className="w-24 h-24 rounded-full bg-gradient-to-r from-violet-500 to-indigo-500 flex items-center justify-center">
                <span className="text-4xl pulse-icon">?</span>
              </div>
              <div className="absolute inset-0 rounded-full bg-violet-500/30 pulse-ring" />
            </div>
