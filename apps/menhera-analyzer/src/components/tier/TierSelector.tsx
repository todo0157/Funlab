...
import { ParsedChat, AnalysisTier, TIER_INFO } from '../../types/menhera';
    ...
    >
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-lg mb-6">
        ...
        <div className="space-y-3 text-sm">
          ...
          <div className="pt-2 border-t border-gray-100 dark:border-gray-700">
            {Object.entries(messageCountBySender).slice(0, 5).map(([name, count]) => (
              <div key={name} className="flex justify-between text-xs mt-1">
                <span className="text-gray-400">{name}</span>
                <span className="text-gray-500">{count.toLocaleString()}개</span>
              </div>
            ))}
            {Object.keys(messageCountBySender).length > 5 && (
              <p className="text-xs text-gray-400 mt-1">
                외 {Object.keys(messageCountBySender).length - 5}명
              </p>
            )}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <motion.button
          ...
          disabled={isLoading}
          className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-lg border-2 border-gray-200 dark:border-gray-700 hover:border-purple-300 dark:hover:border-purple-500 transition-colors text-left disabled:opacity-50"
        >
          ...
          <div className="mt-6 w-full py-3 bg-gradient-to-r from-purple-500 to-pink-500 text-white rounded-xl font-semibold text-center">
            무료로 시작하기
          </div>
        </motion.button>
